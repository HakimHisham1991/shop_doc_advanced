########################## TCL Event Handlers ##########################
#
#  shop_doc_advanced.tcl - 5_axis_dual_table
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Tables.
#
#  Created by HAKIM @ Saturday, March 14 2026, 10:14:21 +0800
...
#
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables
     ...
     set mom_sys_output_file_suffix                "CSV"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "360000000"
  set mom_kin_4th_axis_min_incr                 "0.001"
  set mom_kin_4th_axis_min_limit                "-360000000"
  ...
  set mom_kin_5th_axis_max_limit                "360000000"
  set mom_kin_5th_axis_min_incr                 "0.001"
  set mom_kin_5th_axis_min_limit                "-360000000"
  ...
  set mom_kin_rapid_feed_rate                   "99999"
  ...
  set mom_kin_x_axis_limit                      "9999"
  set mom_kin_y_axis_limit                      "9999"
  set mom_kin_z_axis_limit                      "9999"
...
#=============================================================
  ...
  # Close warning and listing files
   ...
   # Replace NX output file with custom CSV content (single .CSV output)
...
if {[info exists ::env(MOM_CSV_ONLY)] && $::env(MOM_CSV_ONLY) != "0" && $::env(MOM_CSV_ONLY) ne ""} {
   set ::mom_csv_only_mode 1
} else {
   set ::mom_csv_only_mode 0
}
...
# CSV Header: fixed columns first, then mom_ parameter columns
puts $file_handle [join [list \
    ...
    {mom_stepover_variable_tool_dependent_values_source[1]} \
    {mom_stepover_variable_tool_dependent_values[2]} \
    {mom_stepover_variable_tool_dependent_values_source[2]} \
    {mom_stepover_variable_tool_dependent_values[3]} \
    {mom_stepover_variable_tool_dependent_values_source[3]} \
    {mom_stepover_variable_tool_dependent_values[4]} \
    {mom_stepover_variable_tool_dependent_values_source[4]} \
    {mom_stepover_variable_tool_dependent_values[5]} \
    {mom_stepover_variable_tool_dependent_values_source[5]} \
    mom_depth_per_cut \
    mom_cut_level_max_depth \
    mom_depth_of_cut_type \
    ...
    mom_axial_stepover_type \
    mom_axial_stepover_passes \
    mom_axial_stepover_percent \
    mom_cycle_step1 \
    mom_cycle_type \
    mom_helical_ramp_angle \
    mom_vertical_pitch_type \
    mom_vertical_pitch_value \
...
#=============================================================
# Helper procs (shared dispatch logic)
#=============================================================
...
proc pb__ae_std_sot_14 { sot sds_defined sds } {
    set final_ae "N/A"
    switch -- $sot {
        1 {
            if {$sds_defined && $sds == 4} {
...
proc pb__ae_std_sot_145 { sot sds_defined sds step_points_2 } {
    set final_ae "N/A"
    switch -- $sot {
        1 {
            if {$sds_defined && $sds == 4} {
...
# Returns ae for subtypes that only support stepover_type 1 and 5 (e.g. GROOVE_MILLING).
proc pb__ae_std_sot_15 { sot sds_defined sds step_points_2 } {
    set final_ae "N/A"
    switch -- $sot {
        1 {
            if {$sds_defined && $sds == 4} {
...
proc pb__ae_std_sot_1234_m { sot sds_defined sds stepover_var_1 } {
    set final_ae "N/A"
    switch -- $sot {
        1 {
            if {$sds_defined && $sds == 4} {
...
proc pb__ae_std_sot_1234 { sot sds_defined sds } {
    set final_ae "N/A"
    switch -- $sot {
        1 {
            if {$sds_defined && $sds == 4} {
...
proc pb__ae_tool_dia {} {
    if {[info exists ::mom_tool_diameter]} {
        return [pb__round_param_4dec $::mom_tool_diameter]
    }
    return "N/A"
}
...
proc pb__ae_ap_max_scallop_limits { strict_double } {
    set max_val ""
    set any_val 0
    foreach v {::mom_stepover_scallop ::mom_horizonal_limit ::mom_vertical_limit} {
        if {[info exists $v] && [set $v] ne ""} {
            set val [set $v]
            if {$strict_double && ![string is double -strict $val]} { continue }
...
proc pb__ap_planar_mill {} {
    if {![info exists ::mom_depth_of_cut_type]} {
        return "N/A"
    }
    switch -- $::mom_depth_of_cut_type {
        0 -
        4 { return [pb__ae_ap_var mom_cut_level_max_depth] }
        1 -
        2 -
        3 { return "NO DATA" }
    }
...
# Returns ap for GROOVE_MILLING (mill_planar) from axial stepover settings.
proc pb__ap_groove_milling {} {
    if {![info exists ::mom_axial_stepover_type]} {
        return "N/A"
    }
    if {$::mom_axial_stepover_type == 2} {
        if {[info exists ::mom_axial_stepover_passes] && $::mom_axial_stepover_passes ne ""} {
            set n $::mom_axial_stepover_passes
            if {[string is double -strict $n]} {
                return "[expr {int(round($n))}] Passes"
            }
            return "$n Passes"
    ...
    if {$::mom_axial_stepover_type == 3} {
        if {[info exists ::mom_axial_stepover_distance_source] && $::mom_axial_stepover_distance_source == 4
            && [info exists ::mom_axial_stepover_percent] && [info exists ::mom_tool_flute_length]} {
            return [pb__round_param_4dec [expr {double($::mom_axial_stepover_percent) / 100.0 * $::mom_tool_flute_length}]]
        }
        return "N/A"
    }
    if {$::mom_axial_stepover_type == 6} {
        if {![info exists ::mom_axial_stepover_distance_source]} {
            return [pb__ae_ap_var mom_axial_stepover_distance]
        }
        if {[info exists ::mom_axial_stepover_distance] && [info exists ::mom_tool_diameter] && $::mom_tool_diameter != 0} {
            return [pb__round_param_4dec [expr {double($::mom_axial_stepover_distance) / 100.0 * $::mom_tool_diameter}]]
        }
        return "N/A"
    }
    return "N/A"
...
if {[info exists mom_operation_type] && $mom_operation_type ne ""} {
    set operation_type $mom_operation_type
} else {
    set operation_type "N/A"
}

# --- Spindle RPM ---
if {[info exists mom_spindle_rpm] && $mom_spindle_rpm ne ""} {
    set spindle $mom_spindle_rpm
} else {
    set spindle "N/A"
}

# --- Feedrate ---
if {[info exists mom_feed_cut_value] && $mom_feed_cut_value ne ""} {
    set feed $mom_feed_cut_value
} elseif {[info exists mom_feed_rate] && $mom_feed_rate ne ""} {
    set feed $mom_feed_rate
} else {
    set feed "N/A"
}

# --- Cycle Time (per-operation, in minutes) ---
if {[info exists mom_machine_time] && [info exists mom_sys_machine_time]} {
    set cycle_time [format "%.4f" [expr {$mom_machine_time - $mom_sys_machine_time}]]
} else {
    set cycle_time "N/A"
}
...
if {[info exists mom_tool_type] && $mom_tool_type eq "User Defined Mill Tool"} {
    set tool_dia "N/A"
} elseif {[info exists mom_tool_diameter] && $mom_tool_diameter ne ""} {
    set tool_dia $mom_tool_diameter
} else {
    set tool_dia "N/A"
}

# --- mom_stepover_variable_max_min[0] and [1] ---
if {[info exists mom_stepover_variable_max_min(0)] && $mom_stepover_variable_max_min(0) ne ""} {
    set stepover_var_0 [pb__round_param_4dec $mom_stepover_variable_max_min(0)]
} else {
    set stepover_var_0 "N/A"
}
if {[info exists mom_stepover_variable_max_min(1)] && $mom_stepover_variable_max_min(1) ne ""} {
    set stepover_var_1 [pb__round_param_4dec $mom_stepover_variable_max_min(1)]
} else {
    set stepover_var_1 "N/A"
}

# --- mom_tool_pitch ---
if {[info exists mom_tool_pitch] && $mom_tool_pitch ne ""} {
    set tool_pitch_output [pb__round_param_4dec $mom_tool_pitch]
} else {
    set tool_pitch_output "No Thread"
}

# --- max(mom_stepover_variable_tool_dependent_values[0..99]) ---
set max_stepover_var_tool_dep "N/A"
set _max_stepover_var_tool_dep ""
for {set _i 0} {$_i <= 99} {incr _i} {
    if {[info exists mom_stepover_variable_tool_dependent_values($_i)] && $mom_stepover_variable_tool_dependent_values($_i) ne ""} {
        set _v $mom_stepover_variable_tool_dependent_values($_i)
        if {[string is double -strict $_v]} {
            if {$_max_stepover_var_tool_dep eq "" || $_v > $_max_stepover_var_tool_dep} {
                set _max_stepover_var_tool_dep $_v
            }
        }
    }
}
if {$_max_stepover_var_tool_dep ne ""} {
    set max_stepover_var_tool_dep [pb__round_param_4dec $_max_stepover_var_tool_dep]
}

# --- mom_stepover_variable_tool_dependent_values[0..5] and source[0..5] ---
set stepover_var_tool_dep_cols {}
set stepover_var_tool_dep_src_cols {}
for {set _i 0} {$_i <= 5} {incr _i} {
    if {[info exists mom_stepover_variable_tool_dependent_values($_i)] && $mom_stepover_variable_tool_dependent_values($_i) ne ""} {
        lappend stepover_var_tool_dep_cols [pb__round_param_4dec $mom_stepover_variable_tool_dependent_values($_i)]
    } else {
        lappend stepover_var_tool_dep_cols "N/A"
    }
    if {[info exists mom_stepover_variable_tool_dependent_values_source($_i)] && $mom_stepover_variable_tool_dependent_values_source($_i) ne ""} {
        lappend stepover_var_tool_dep_src_cols $mom_stepover_variable_tool_dependent_values_source($_i)
    } else {
        lappend stepover_var_tool_dep_src_cols "N/A"
    }
}

# --- mom_step_points[2] ---
set step_points_2 "N/A"
catch {
    if {[info exists mom_step_points(2)] && $mom_step_points(2) ne ""} {
        set step_points_2 [pb__round_param_4dec $mom_step_points(2)]
    }
}

# --- mom_stepover_distance: use 2nd recorded value for all types that collect a list ---
if {[info exists ::path_stepover_distance_list] && [llength $::path_stepover_distance_list] >= 2} {
    set stepover_dist_output [lindex $::path_stepover_distance_list 1]
} elseif {[info exists ::path_stepover_distance_list] && [llength $::path_stepover_distance_list] == 1} {
    set stepover_dist_output [lindex $::path_stepover_distance_list 0]
} else {
    set stepover_dist_output [pb__mom_var_or_na mom_stepover_distance]
}

# --- path_stepover_distance_list: first 3 elements as separate columns (N/A if not exist) ---
set path_stepover_1 "N/A"
set path_stepover_2 "N/A"
set path_stepover_3 "N/A"
if {[info exists ::path_stepover_distance_list] && [llength $::path_stepover_distance_list] > 0} {
    set path_stepover_1 [lindex $::path_stepover_distance_list 0]
    if {[llength $::path_stepover_distance_list] >= 2} {
        set path_stepover_2 [lindex $::path_stepover_distance_list 1]
    }
    if {[llength $::path_stepover_distance_list] >= 3} {
        set path_stepover_3 [lindex $::path_stepover_distance_list 2]
    }
}
...
# ============================================================
# FINAL Ae — table-driven dispatch
# ============================================================

# --- mill_planar Ae dispatch table ---
# Keys: subtype (uppercase). Values: handler script.
...
#   std_sot_1234_m   -> call pb__ae_std_sot_1234_m (covers sot 1/2/3/4 manual)
#   std_sot_15     -> call pb__ae_std_sot_15 (covers sot 1/5, e.g. GROOVE_MILLING)
...
array set ae_mill_planar {
    ...
    FACE_MILL_SPIRAL       std_sot_14
    FACE_MILL_ZIGZAG       std_sot_145
    2D_WALL_MILL           tool_dia
    FLOOR_WALL             std_sot_1234
    POCKETING              std_sot_1234
    WALL_PROFILING         std_sot_1234
    WALL_FLOOR_PROFILING   std_sot_1234
    PLANAR_PROFILING       planar_profiling
    PLANAR_MILL            std_sot_1234
    ...
    if {[info exists mom_template_type] && $mom_template_type eq "mill_planar"} {
        set st ""
        if {[info exists mom_template_subtype] && $mom_template_subtype ne ""} {
            set st [string toupper $mom_template_subtype]
        }
        if {[info exists ae_mill_planar($st)]} {
            set _handler $ae_mill_planar($st)
            switch -- $_handler {
                ...
                std_sot_1234     { set final_ae [pb__ae_std_sot_1234 $sot $sds_defined $sds] }
                std_sot_15       { set final_ae [pb__ae_std_sot_15 $sot $sds_defined $sds $step_points_2] }
            }
        }
    }
}

# --- mill_contour Ae dispatch table ---
# Tokens:
#   stepover_dist  -> ae = mom_stepover_distance (rounded)
#   tool_dia       -> ae = mom_tool_diameter (rounded)
#   path_step_2    -> ae = path_stepover_2 (already resolved list value)
#   wall_incr      -> ae = mom_wall_increment
#   no_data        -> ae = "NO DATA"
...
array set ae_mill_contour {
    ...
    if {[info exists mom_template_type] && $mom_template_type eq "mill_contour"} {
        set st ""
        if {[info exists mom_template_subtype] && $mom_template_subtype ne ""} {
            set st $mom_template_subtype
        }
...
array set ae_mill_multiaxis {
    ...
    if {[info exists mom_template_type] && $mom_template_type eq "mill_multi-axis"} {
        set st ""
        if {[info exists mom_template_subtype] && $mom_template_subtype ne ""} {
            set st $mom_template_subtype
        }
        if {[info exists ae_mill_multiaxis($st)]} {
...
# --- hole_making Ae dispatch table ---
# Most subtypes: ae = tool_diameter; THREAD_MILLING uses stepover-type logic

array set ae_hole_making {
    SPOT_DRILLING   tool_dia
    DRILLING        tool_dia
    BORING_REAMING  tool_dia
    TAPPING         tool_dia
    DEEP_HOLE_DRILLING tool_dia
    HOLE_MILLING    tool_dia
    THREAD_MILLING  thread_mill
}

catch {
    if {[info exists mom_template_type] && $mom_template_type eq "hole_making"} {
        set st ""
        if {[info exists mom_template_subtype] && $mom_template_subtype ne ""} {
            set st $mom_template_subtype
        }
        if {[info exists ae_hole_making($st)]} {
            switch -- $ae_hole_making($st) {
                tool_dia {
                    set final_ae [pb__ae_tool_dia]
                }
                thread_mill {
                    if {[info exists mom_stepover_type] && $mom_stepover_type ne ""} {
                        switch -- $mom_stepover_type {
                            1 {
                                if {[info exists mom_stepover_distance] && $mom_stepover_distance != 0} {
                                    set final_ae [pb__round_param_4dec $mom_stepover_distance]
                                } elseif {[info exists mom_tool_pitch] && $mom_tool_pitch ne ""} {
                                    set final_ae [pb__round_param_4dec $mom_tool_pitch]
                                }
                            }
                            3 {
                                if {$max_stepover_var_tool_dep ne "N/A"} {
                                    set final_ae $max_stepover_var_tool_dep
                                }
                            }
                            8 {
                                if {$stepover_var_1 ne "N/A"} {
...
# ============================================================
# FINAL Ap — table-driven dispatch
# ============================================================

# --- mill_planar Ap dispatch table ---
# Tokens:
#   cut_level_dist  -> ap = mom_cut_level_distance
#   depth_per_cut   -> ap = mom_depth_per_cut
...
array set ap_mill_planar {
    ...
    2D_WALL_MILL         face_mill_midpass
    FLOOR_WALL           depth_per_cut
    POCKETING            depth_per_cut
    WALL_PROFILING       depth_per_cut
    WALL_FLOOR_PROFILING depth_per_cut
    PLANAR_PROFILING     planar_mill
    PLANAR_MILL          planar_mill
    GROOVE_MILLING       groove_milling
    PLANAR_DEBURRING     no_data
    MILL_CONTROL         no_data
    FLOOR_FACING         depth_per_cut
    FACE_MILLING_MANUAL  depth_per_cut
}
    ...
    if {[info exists mom_template_type] && $mom_template_type eq "mill_contour"} {
        ...
        if {[info exists ap_mill_contour($st)]} {
            switch -- $ap_mill_contour($st) {
                ...
                max_scallop      { set final_ap [pb__ae_ap_max_scallop_limits 0] }
            }
        }
    }
}
...
# --- hole_making Ap dispatch table ---
# Tokens:
#   flute_length     -> ap = mom_tool_flute_length (base)
#   drill_with_peck  -> flute_length base, overridden by cycle_step1 or depth_increment_distance_source==4
#   vertical_pitch   -> HOLE_MILLING special logic
#   flute_length     -> basic flute only

array set ap_hole_making {
    SPOT_DRILLING      flute_only
    DRILLING           drill_with_peck
    BORING_REAMING     flute_only
    TAPPING            drill_with_peck
    DEEP_HOLE_DRILLING flute_only
    HOLE_MILLING       vertical_pitch
    THREAD_MILLING     flute_only
}
    ...
    if {[info exists mom_template_type] && $mom_template_type eq "hole_making"} {
        ...
        if {[info exists ap_hole_making($st)]} {
            switch -- $ap_hole_making($st) {
                flute_only {
                    set final_ap [pb__ae_ap_var mom_tool_flute_length]
                }
                drill_with_peck {
                    # Base: flute length
                    set final_ap [pb__ae_ap_var mom_tool_flute_length]
                    ...
                    if {[info exists mom_cycle_step1] && $mom_cycle_step1 != 0} {
                        set final_ap [pb__round_param_4dec $mom_cycle_step1]
                    }
                    # Override: depth_increment_distance as % of tool_diameter
                    if {[info exists mom_depth_increment_distance_source] && $mom_depth_increment_distance_source == 4} {
                        if {[info exists mom_depth_increment_distance] && [info exists mom_tool_diameter] && $mom_tool_diameter != 0} {
                            set final_ap [pb__round_param_4dec [expr {double($mom_depth_increment_distance) / 100.0 * $mom_tool_diameter}]]
                        }
                    }
                }
                vertical_pitch {
                    # HOLE_MILLING
                    set vpt -1
                    if {[info exists mom_vertical_pitch_type]} { set vpt $mom_vertical_pitch_type }
                    if {$vpt == 0} {
                        set vps_exists [info exists mom_vertical_pitch_value_source]
                        if {!$vps_exists || $mom_vertical_pitch_value_source == 0} {
                            set final_ap [pb__ae_ap_var mom_vertical_pitch_value]
                        } elseif {$mom_vertical_pitch_value_source == 4} {
                            if {[info exists mom_vertical_pitch_value] && [info exists mom_tool_diameter] && $mom_tool_diameter != 0} {
                                set final_ap [pb__round_param_4dec [expr {double($mom_vertical_pitch_value) / 100.0 * $mom_tool_diameter}]]
                            }
                        }
                    } else {
                        set final_ap [pb__ae_ap_var mom_tool_flute_length]
...
# ============================================================
# CSV row assembly and output
# ============================================================

incr csv_row_counter

set tool_ref_number ""
if {[info exists mom_tool_number] && $mom_tool_number ne "" && [string is double -strict $mom_tool_number]} {
    set _tn [expr {int(round($mom_tool_number))}]
    if {$_tn >= 0} {
        set tool_ref_number [format "T%02d" $_tn]
    }
}

set csv_part_number ""
if {[info exists mom_part_name] && [string trim $mom_part_name] ne ""} {
    set csv_part_number [file rootname [file tail [string trim $mom_part_name]]]
}
...
set row_fields [list \
    ...
    [pb__mom_var_or_na mom_stock_floor] \
    [pb__mom_var_or_na mom_wall_stock] \
    [pb__mom_var_or_na mom_z_depth_offset] \
    $stepover_dist_output \
    $path_stepover_1 \
    $path_stepover_2 \
    $path_stepover_3 \
    ...
    [lindex $stepover_var_tool_dep_src_cols 1] \
    [lindex $stepover_var_tool_dep_cols 2] \
    [lindex $stepover_var_tool_dep_src_cols 2] \
    [lindex $stepover_var_tool_dep_cols 3] \
    [lindex $stepover_var_tool_dep_src_cols 3] \
    [lindex $stepover_var_tool_dep_cols 4] \
    [lindex $stepover_var_tool_dep_src_cols 4] \
    [lindex $stepover_var_tool_dep_cols 5] \
    [lindex $stepover_var_tool_dep_src_cols 5] \
    [pb__mom_var_or_na mom_depth_per_cut] \
    [pb__mom_var_or_na mom_cut_level_max_depth] \
    [pb__mom_var_or_na mom_depth_of_cut_type] \
    ...
    [pb__mom_var_or_na mom_axial_stepover_type] \
    [pb__mom_var_or_na mom_axial_stepover_passes] \
    [pb__mom_var_or_na mom_axial_stepover_percent] \
    [pb__mom_var_or_na mom_cycle_step1] \
    [pb__mom_var_or_na mom_cycle_type] \
    [pb__mom_var_or_na mom_helical_ramp_angle] \
    [pb__mom_var_or_na mom_vertical_pitch_type] \
    [pb__mom_var_or_na mom_vertical_pitch_value] \
...
#=============================================================
proc pb__mom_var_or_na { var_name } {
#=============================================================
# Returns value of mom variable (rounded to 4 dec if numeric), or "N/A" if not available.
...
#=============================================================
proc pb__round_param_4dec { value } {
#=============================================================
# Rounds numeric cutting parameters to 4 decimal places.
# Returns N/A and other non-numeric values unchanged.
...
#=============================================================
proc pb__trace_stepover { name1 name2 op } {
#=============================================================
# Tcl variable trace callback — fires every time mom_stepover_distance
# is written (including during NX parameter loading, before motion events).
    upvar #0 mom_stepover_distance val
    if {![info exists val] || $val eq ""} { return }
    set rounded [pb__round_param_4dec $val]
    if {![info exists ::path_stepover_distance_list]} { set ::path_stepover_distance_list {} }
